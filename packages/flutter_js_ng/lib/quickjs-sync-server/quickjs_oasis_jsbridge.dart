...
class QuickJsService extends JavascriptRuntime {
  ...
  JsEvalResult evaluate(String code, {String? sourceUrl}) {
    final request = SyncHttpClient.postUrl(Uri.http(
  ...
  Future<JsEvalResult> evaluateAsync(String code, {String? sourceUrl}) async {
    final String strResult = await _flutterJs.eval(code);
    return JsEvalResult(
      strResult,
      null,
...
Future<void> startQuickJsServer(SendPort sendPort) async {
  final server = QuickJsSyncServer();
  ...
  Future<void> _handleReq(HttpRequest request) async {
    ...
    if (path == 'callDart') {
      try {
        ...
        final String message = await utf8.decoder.bind(request).join();
        final String? idEngine = request.uri.queryParameters['id'];
        final String? channel = request.uri.queryParameters['channel'];
